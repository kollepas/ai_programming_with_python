...
def calculates_results_stats(results_dic):
    ...
    # this function
    n_images, n_correct_dogs, n_dogs_img, n_correct_notdogs, n_notdogs_img, \
    n_correct_breed, n_match = 0, 0, 0, 0, 0, 0, 0
    ...
    for key, value in results_dic.items():
        ...
        if value[2] == 1 and value[3] == 1:
            n_correct_breed += 1
        # Y: Number of label matches (Optional)
        if value[2] == 1:
            n_match += 1
    # Objective _1_a and Objective _2
    try:
        pct_correct_dogs = round(n_correct_dogs / n_dogs_img * 100, 1)
        pct_correct_breed = round(n_correct_breed / n_dogs_img * 100, 1)
    except ZeroDivisionError:
        pct_correct_dogs, pct_correct_breed = 0, 0
    # Objective _1_b
    try:
        pct_correct_notdogs = round(n_correct_notdogs / n_notdogs_img * 100, 1)
    except ZeroDivisionError:
        pct_correct_notdogs = 0
    # (Optional): Percentage Label Matches
    try:
        pct_match = round(n_match / n_images * 100, 1)
    except ZeroDivisionError:
        pct_match = 0
    # Fill results in directory
